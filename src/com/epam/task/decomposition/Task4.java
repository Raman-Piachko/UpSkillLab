...
public class Task4 {
    public static void main(String[] args) {
        ...
        findPointsWithMaxDistance(points);
    }

    private static int[][] createPointsArray(int numberOfPoints) {
    ...
    private static void findPointsWithMaxDistance(int[][] points) {
        ...
        for (int i = 0; i < points.length; i++) {
            for (int j = i; j < points.length - 1; j++) {
                ...
                if (distance > maxDistance) {
                    ...
                    firstPoint = String.format("(%d:%d)", points[i][0], points[i][1]);
                    secondPoint = String.format("(%d:%d)", points[j + 1][0], points[j + 1][1]);
                }
            }
        }
    ...
    private static double calculateDistance(double pointX, double point1X, double pointY, double point1Y) {
